import React, { useState } from "react";

// ✏️ EDITA TUS REDES SOCIALES AQUÍ
const SOCIAL_LINKS = [
  {
    name: "GitHub",
    url: "https://github.com/jordanhinojoza",  // 👈 Tu usuario
    icon: (
      <svg viewBox="0 0 24 24" className="w-5 h-5 fill-current">
        <path d="M12 .297c-6.63 0-12 5.373-12 12 0 5.303 3.438 9.8 8.205 11.385.6.113.82-.258.82-.577 0-.285-.01-1.04-.015-2.04-3.338.724-4.042-1.61-4.042-1.61C4.422 18.07 3.633 17.7 3.633 17.7c-1.087-.744.084-.729.084-.729 1.205.084 1.838 1.236 1.838 1.236 1.07 1.835 2.809 1.305 3.495.998.108-.776.417-1.305.76-1.605-2.665-.3-5.466-1.332-5.466-5.93 0-1.31.465-2.38 1.235-3.22-.135-.303-.54-1.523.105-3.176 0 0 1.005-.322 3.3 1.23.96-.267 1.98-.399 3-.405 1.02.006 2.04.138 3 .405 2.28-1.552 3.285-1.23 3.285-1.23.645 1.653.24 2.873.12 3.176.765.84 1.23 1.91 1.23 3.22 0 4.61-2.805 5.625-5.475 5.92.42.36.81 1.096.81 2.22 0 1.606-.015 2.896-.015 3.286 0 .315.21.69.825.57C20.565 22.092 24 17.592 24 12.297c0-6.627-5.373-12-12-12" />
      </svg>
    ),
    color: "hover:text-white hover:border-white/40",
  },
  {
    name: "LinkedIn",
    url: "https://linkedin.com/in/jordanhinojoza",  // 👈 Tu perfil
    icon: (
      <svg viewBox="0 0 24 24" className="w-5 h-5 fill-current">
        <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z" />
      </svg>
    ),
    color: "hover:text-blue-400 hover:border-blue-400/40",
  },
  {
    name: "Email",
    url: "mailto:jordan@example.com",  // 👈 Tu correo
    icon: (
      <svg viewBox="0 0 24 24" className="w-5 h-5 fill-none stroke-current stroke-2" strokeLinecap="round" strokeLinejoin="round">
        <path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z" />
        <polyline points="22,6 12,13 2,6" />
      </svg>
    ),
    color: "hover:text-cyan-400 hover:border-cyan-400/40",
  },
];

const Contact = () => {
  const [form, setForm] = useState({ name: "", email: "", message: "" });
  const [sent, setSent] = useState(false);

  const handleChange = (e) =>
    setForm({ ...form, [e.target.name]: e.target.value });

  // 💡 Conecta esto con Formspree, EmailJS, o tu propio endpoint
  const handleSubmit = (e) => {
    e.preventDefault();
    // ✏️ REEMPLAZA con tu lógica de envío de formulario
    console.log("Form submitted:", form);
    setSent(true);
    setTimeout(() => setSent(false), 4000);
    setForm({ name: "", email: "", message: "" });
  };

  return (
    <section id="contact" className="py-24 px-6">
      <div className="max-w-4xl mx-auto">
        {/* Header */}
        <div className="mb-12">
          <p className="text-cyan-500 font-mono text-sm mb-2">// contacto</p>
          <h2 className="text-3xl md:text-4xl font-bold text-white">
            Hablemos
          </h2>
          <div className="w-12 h-0.5 bg-cyan-500 mt-3" />
          <p className="text-slate-400 mt-4 max-w-xl">
            ¿Tienes un proyecto en mente, una pregunta o simplemente quieres conectar?
            No dudes en escribirme.
          </p>
        </div>

        <div className="grid md:grid-cols-2 gap-10">
          {/* Formulario */}
          <form onSubmit={handleSubmit} className="space-y-4">
            {[
              { name: "name",    label: "Nombre",  type: "text",  placeholder: "Jordan Hinojoza" },
              { name: "email",   label: "Email",   type: "email", placeholder: "jordan@email.com" },
            ].map(({ name, label, type, placeholder }) => (
              <div key={name}>
                <label className="block text-xs text-slate-500 font-mono mb-1">{label}</label>
                <input
                  type={type}
                  name={name}
                  value={form[name]}
                  onChange={handleChange}
                  placeholder={placeholder}
                  required
                  className="w-full bg-slate-800/60 border border-slate-700 text-slate-300 placeholder-slate-600 rounded-lg px-4 py-2.5 text-sm font-mono focus:outline-none focus:border-cyan-500/60 transition-colors"
                />
              </div>
            ))}

            <div>
              <label className="block text-xs text-slate-500 font-mono mb-1">Mensaje</label>
              <textarea
                name="message"
                value={form.message}
                onChange={handleChange}
                rows={5}
                placeholder="Hola Jordan, me gustaría hablar sobre..."
                required
                className="w-full bg-slate-800/60 border border-slate-700 text-slate-300 placeholder-slate-600 rounded-lg px-4 py-2.5 text-sm font-mono focus:outline-none focus:border-cyan-500/60 transition-colors resize-none"
              />
            </div>

            <button
              type="submit"
              className="w-full py-3 bg-cyan-500 hover:bg-cyan-400 text-slate-950 font-semibold rounded-lg transition-all font-mono text-sm hover:shadow-lg hover:shadow-cyan-500/30"
            >
              {sent ? "✓ Mensaje enviado" : "Enviar mensaje →"}
            </button>
          </form>

          {/* Redes sociales */}
          <div className="flex flex-col justify-center gap-4">
            <p className="text-slate-500 font-mono text-xs uppercase tracking-wider">
              También puedes encontrarme en:
            </p>
            {SOCIAL_LINKS.map(({ name, url, icon, color }) => (
              <a
                key={name}
                href={url}
                target={name !== "Email" ? "_blank" : undefined}
                rel="noopener noreferrer"
                className={`flex items-center gap-3 p-4 bg-slate-800/50 border border-slate-700/50 rounded-xl text-slate-400 transition-all ${color} hover:bg-slate-800`}
              >
                {icon}
                <span className="font-mono text-sm">{name}</span>
              </a>
            ))}
          </div>
        </div>
      </div>
    </section>
  );
};

export default Contact;
